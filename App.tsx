            ...
            <>
              ...
              <main className="flex-1 md:ml-64 p-4 md:p-8">
                ...
                <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex justify-around p-2">
                   <button onClick={() => setCurrentTab('dashboard')} className={`p-2 flex flex-col items-center ${currentTab === 'dashboard' ? 'text-blue-600' : 'text-slate-400'}`}>
                      <span className="text-xl">📊</span>
                      <span className="text-[10px] font-bold">Painel</span>
                   </button>
                   <button onClick={() => setCurrentTab('patients')} className={`p-2 flex flex-col items-center ${currentTab === 'patients' ? 'text-blue-600' : 'text-slate-400'}`}>
                      <span className="text-xl">👥</span>
                      <span className="text-[10px] font-bold">Pacientes</span>
                   </button>
                   <button onClick={() => setCurrentTab('whatsapp')} className={`p-2 flex flex-col items-center ${currentTab === 'whatsapp' ? 'text-blue-600' : 'text-slate-400'}`}>
                      <span className="text-xl">💬</span>
